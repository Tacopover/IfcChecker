...
describe("visibilityBytes", () => {
  it("writes one byte per slot, keyed on the slot's express id", () => {
    ...
    const bytes = visibilityBytes(expressIds, (id) => id !== 200, 8);
    expect([...bytes.slice(0, 3)]).toEqual([255, 0, 255]);
  });
  ...
  it("hides every slot belonging to a hidden element", () => {
    ...
    const bytes = visibilityBytes(expressIds, (id) => id !== 100, 8);
    expect([...bytes.slice(0, 3)]).toEqual([0, 0, 255]);
  });

  it("fills the whole texture, so slots past the end read as hidden", () => {
    const bytes = visibilityBytes(new Uint32Array([100]), () => true, 8);
