...
/**
 * The visibility byte array for a batch. Tri-state per slot: 0 hidden, 1
 * visible, 2 visible-and-highlighted (see visibility.ts's `visibilityCode`).
 * The vertex shader throws hidden vertices outside the clip volume so their
 * triangles are dropped before rasterization rather than discarded per
 * fragment; a highlighted slot draws normally but the fragment stage tints it.
 */
export function visibilityBytes(
  expressIdBySlot: Uint32Array,
  slotState: (expressId: number) => 0 | 1 | 2,
  width = 2048
  ...
  for (let slot = 0; slot < expressIdBySlot.length; slot++) {
    bytes[slot] = slotState(expressIdBySlot[slot]);
  }
  return bytes;
}
