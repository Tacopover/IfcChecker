...
void main() {
  ...
  // Highlight-only mode tints a *set* of elements (visible == 2u) without
  // hiding anything else; the single inspected element (uSelectedSlot) is a
  // separate, stronger tint that wins even inside a highlighted set.
  vec4 baseColor = visible == 2u ? vec4(mix(aColor.rgb, vec3(1.0, 0.75, 0.2), 0.55), aColor.a) : aColor;
  vColor = uPickPass
    ? pickColorFor(aSlot)
    : (aSlot == uSelectedSlot ? vec4(1.0, 0.62, 0.16, aColor.a) : baseColor);
...
export class ViewerRenderer {
   ...
   * `slotState` returns 0 hidden / 1 visible / 2 visible-and-highlighted (see
   * visibility.ts's `visibilityCode`).
   */
  setVisibility(slotState: (modelKey: string, expressId: number) => 0 | 1 | 2): void {
    const gl = this.gl;
    for (const batch of this.batches) {
      ...
      const bytes = visibilityBytes(
        ...
        (expressId) => slotState(batch.modelKey, expressId),
        VISIBILITY_ROW
      );
